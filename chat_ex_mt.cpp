...
void setBuffers() {
    ...
    buffer = cl::Buffer(context, CL_MEM_READ_WRITE, (size_t)NUM_PARTICLES*NUM_PARTICLES*3, NULL, &err);
    if (err != CL_SUCCESS) {
        std::cerr << "Failed to create OpenCL buffer for vel" << std::endl;
        exit(EXIT_FAILURE);
    }
...
void updatePos(float dt) {
    ...
    if(mode==2){
        ...
        off=1;
        int total_threads=NUM_PARTICLES*(NUM_PARTICLES-1)/2;
        GROUP_SIZE=(total_threads+LOCAL_SIZE-1)/LOCAL_SIZE;
    }
    kernel.setArg(2+off, NUM_PARTICLES);
    kernel.setArg(3+off, dt);
    if(mode==1){
...
int main(int argc, char* argv[]) {
    ...
    if(mode==1){
        ...
        src_code = load_from_file("triangularKernel.cl");
    }
    else{
